//
//  MLPModalTableTableViewController.m
...
@interface MLPModalTableTableViewController ()
{
...
@synthesize selectedDuplicateAddressesArray;
...
- (void)viewDidLoad
{
    ...
    self.selectedDuplicateAddressesArray = [[NSMutableArray alloc] init];
...
-(void)selectAllTableViewItems:(UITableView *)tableView{
    for (int i = 0 ; i < [self.tableView numberOfRowsInSection:0]; i ++)
        if(![selectedDuplicateAddressesArray containsObject:[NSNumber numberWithInt:i]])
...
-(void)resetAllTableViewItems:(UITableView *)tableView{
    //removing all selected objects
    isCellSelected = 0;
    [selectedDuplicateAddressesArray removeAllObjects];
    [tableView reloadData];
    if([self.mlpDataDelegate respondsToSelector:@selector(addressCount:from:)])
        [self.mlpDataDelegate addressCount:0 from:[tableView numberOfRowsInSection:0]];
}
#pragma mark -
...
-(NSString*)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    ...
    if([popUpScreenTitle isEqualToString:MLP_SCREEN])
        ...
        return REMOVE_ADDRESS_TABLE_HEADER;

//    else
//        return TABLE_HEADER_FOR_DUPLICATE;
}
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UILabel* label=[[UILabel alloc]initWithFrame:CGRectMake(1, 0, 750, 40)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([popUpScreenTitle isEqualToString:MLP_SCREEN])
    {
        MLPTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"MLPTableViewCell" owner:self options:nil] objectAtIndex:0];
        }
        ...
        [cell.add3 setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.add3 setTextColor:[UIColor grayColor]];
        
        [cell.primarySpeLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.secondarySpecLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.masterIDLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.npiLabel setFont:[UIFont fontWithName:@"Roboto-Bold" size:14.0]];
        [cell.npiAnswerLabel setFont:[UIFont fontWithName:@"Roboto-Bold" size:14.0]];
        [cell.secSpeAnsLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.nameLabel setFont:[UIFont fontWithName:@"Roboto-Bold" size:14.0]];
        ...
        if([dictionary objectForKey:@"FRST_NM"])
            name = [NSString stringWithFormat:@"%@ %@ %@", [dictionary objectForKey:@"FRST_NM"], [dictionary objectForKey:@"MIDL_NM"], [dictionary objectForKey:@"LAST_NM"]];
        cell.add1.text = name;//[dictionary objectForKey:@"name"];
        if([dictionary objectForKey:@"PRIMARY_SPECIALTY_NM"])
            cell.add2.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"PRIMARY_SPECIALTY_NM"]];
        else
            cell.add2.text = @"-";
        //cell.add3.text = [dictionary objectForKey:@"bpId"];
        //int temp = (int)[dictionary objectForKey:@"bpId"];
        //NSString *strFromInt = [NSString stringWithFormat:@"%d",temp];
        if([dictionary objectForKey:@"BP_ID"])
            cell.add3.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"BP_ID"]];
        
        if([dictionary objectForKey:@"SECONDARY_SPECIALTY_NM"])
            cell.secSpeAnsLabel.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"SECONDARY_SPECIALTY_NM"]];
        else
            cell.secSpeAnsLabel.text =@"-";
        //temp = (int)[dictionary objectForKey:@"npiNumber"];
        //strFromInt = [NSString stringWithFormat:@"%d",temp];
        if([dictionary objectForKey:@"NPI_NUM"])
        cell.npiAnswerLabel.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"NPI_NUM"]];
    ...
    else
    {
        ...
        DuplicateAddressTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"DuplicateAddressTableViewCell" owner:self options:nil] objectAtIndex:0];
        }
...
//        }
        ...
        [cell.masterIdAns setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.masterIdAns setTextColor:[UIColor blackColor]];
        
        [cell.address setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.addressType setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
        [cell.addressTypeAns setFont:[UIFont fontWithName:@"Roboto-Regular" size:14.0]];
                //    [cell.nameLabel setTextColor:THEME_COLOR];
...
//        cell.selectionStyle = UITableViewCellSelectionStyleNone;
 //       [cell setAccessoryType:UITableViewCellAccessoryNone];
        ...
        NSDictionary *dictionary = dataArray[indexPath.row];
        if([dictionary objectForKey:@"bpaId"])
            cell.masterIdAns.text = [dictionary objectForKey:@"bpaId"];
       
        if([dictionary objectForKey:@"addrUsageType"])
        {
            NSString *addressType = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"addrUsageType"]];
            if([addressType isEqualToString:@"<null>"])
            {
                cell.addressTypeAns.text = @"";
            }
            else
            {
                cell.addressTypeAns.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"addrUsageType"]];
            }
        }
        if([dictionary objectForKey:@"address"])
            cell.address.text = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"address"]];
        
        for(int arrayIndex = 0; arrayIndex < selectedDuplicateAddressesArray.count; arrayIndex++)
        {
            NSNumber *valueToDeselect = [selectedDuplicateAddressesArray objectAtIndex:arrayIndex];
            if([valueToDeselect intValue] == indexPath.row)
            {
                cell.accessoryType = UITableViewCellAccessoryCheckmark;
            }
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([popUpScreenTitle isEqualToString:DUPLICATE_ADDRESS_SCREEN])
    {
        cell = (DuplicateAddressTableViewCell*) [tableView cellForRowAtIndexPath:indexPath];
        ...
        if (cell.accessoryType == UITableViewCellAccessoryCheckmark)
        {
            ...
            for(int arrayIndex = 0; arrayIndex < selectedDuplicateAddressesArray.count; arrayIndex++)
            {
                NSNumber *valueToDeselect = [selectedDuplicateAddressesArray objectAtIndex:arrayIndex];
                if([valueToDeselect intValue] == indexPath.row)
                    [selectedDuplicateAddressesArray removeObjectAtIndex:arrayIndex];
            }
            
            //check function here
            if([self.mlpDataDelegate respondsToSelector:@selector(addressCount:from:)]){
                //NSLog(@"marked none %d/%d",(int)[selectedDuplicateAddressesArray count],(int)[tableView numberOfRowsInSection:0]);
                [self.mlpDataDelegate addressCount:[selectedDuplicateAddressesArray count] from:[tableView numberOfRowsInSection:0]];
            }
        }
        else
        {
            ...
            if(itemExists == NO)
                [selectedDuplicateAddressesArray addObject:[NSNumber numberWithInt:(int)indexPath.row]];
            
            //check function here
            if([self.mlpDataDelegate respondsToSelector:@selector(addressCount:from:)]){
                //NSLog(@"Checkmarked %d/%d",(int)[selectedDuplicateAddressesArray count],(int)[tableView numberOfRowsInSection:0]);
                [self.mlpDataDelegate addressCount:[selectedDuplicateAddressesArray count] from:[tableView numberOfRowsInSection:0]];
            }
        }
    }
    ...
    {
        cell = (MLPTableViewCell*) [tableView cellForRowAtIndexPath:indexPath];
    ...
    if([[rowParameters objectForKey:ACCESSORY] isEqual:[NSNumber numberWithInteger:UITableViewCellAccessoryCheckmark]])
    {
        ...
        for (int row = 0; row < [rowTitleArray count]; row++)
        {
            if([str_cellTitle isEqualToString:[rowTitleArray objectAtIndex:row]])
            {
                ...
                if([popUpScreenTitle isEqualToString:DUPLICATE_ADDRESS_SCREEN])
                {
                    DuplicateAddressTableViewCell * cell = (DuplicateAddressTableViewCell*) [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:indexPath.section]];
                    [cell setAccessoryType:UITableViewCellAccessoryNone];
                ...
                {
                    MLPTableViewCell * cell = (MLPTableViewCell*) [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:indexPath.section]];
                    [cell setAccessoryType:UITableViewCellAccessoryNone];
                    NSMutableDictionary *rowParameters = [[self.inputTableDataDict objectForKey:[self.sectionArray objectAtIndex:indexPath.section]] objectForKey:[rowTitleArray objectAtIndex:row]];
                    [rowParameters setObject:[NSNumber numberWithInteger:ACCESSORY_STATE_UNSELECTED] forKey:ACCESSORY_STATE_VALUE];
                }
    ...
    //repace the below variables with data from selected cell
    if([popUpScreenTitle isEqualToString:MLP_SCREEN])
    {
        MLPTableViewCell * mlpTableViewCell = (MLPTableViewCell*) [tableView cellForRowAtIndexPath:indexPath];
        NSString *name = mlpTableViewCell.add1.text;
        NSString *primarySpeciality = mlpTableViewCell.add2.text;
        NSString *secondarySpeciality = mlpTableViewCell.secSpeAnsLabel.text;
        NSString *masterID = mlpTableViewCell.add3.text;
        NSString *npi = mlpTableViewCell.npiAnswerLabel.text;

        if ([self.mlpDataDelegate respondsToSelector:@selector(callFlagScreenwithName:primarySpeciality:secondarySpeciality:masterID:andNPI:)]) {
            [self.mlpDataDelegate callFlagScreenwithName:name primarySpeciality:primarySpeciality secondarySpeciality:secondarySpeciality masterID:masterID andNPI:npi];
        }
    }
